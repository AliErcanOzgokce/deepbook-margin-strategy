module strategy_wrapper::strategy_wrapper {
    ...
    /// Deploy borrowed funds to DeepBook margin pool
    public fun deploy_to_deepbook<P, T>(
        strategy_cap: &StrategyOwnerCap<P>,
        lending_market: &mut LendingMarket<P>,
        margin_pool: &mut MarginPool<T>,
        ...
        assert_current_version(strategy_cap);
        let borrowed_coin = borrow_from_obligation<P, T>(
            strategy_cap,
            lending_market,
            reserve_array_index,
            clock,
    ...
    /// Unwind DeepBook position and repay SuiLend obligation
    #[allow(lint(self_transfer))]
    public fun unwind_from_deepbook<P, T>(
        strategy_cap: &StrategyOwnerCap<P>,
        lending_market: &mut LendingMarket<P>,
        margin_pool: &mut MarginPool<T>,
        margin_registry: &MarginRegistry,
        ...
        assert_current_version(strategy_cap);
        let mut withdrawn_coin = margin_pool::withdraw<T>(
            margin_pool,
            margin_registry,
            supplier_cap,
            withdraw_amount,
        ...
        if (withdrawn_coin.value() > 0) {
            transfer::public_transfer(withdrawn_coin, ctx.sender());
        } else {
            coin::destroy_zero(withdrawn_coin);
        };
    }
